...
import { getFSLSMVectorAction, applyBehavioralSignalsAction, FSLSMVectorResponse } from "@/app/actions/profile";

interface NeuroSidebarProps {
  isOpen: boolean;
  refreshTrigger: number; // Increment to force a refetch
}

export function NeuroSidebar({ isOpen, refreshTrigger }: NeuroSidebarProps) {
  ...
  const handleNudge = async (signal: string) => {
    try {
      // Optimitistic wait UI
      const updated = await applyBehavioralSignalsAction([signal]);
      setVector(updated);
    } catch (err) {
  ...
  const renderDimensionBar = (dim: string, val: number, leftLabel: string, rightLabel: string) => {
    // val is [-1, 1], map to [0, 100]%
    const percent = ((val + 1) / 2) * 100;
    
    return (
      <div className="mb-4">
        <div className="flex justify-between text-xs font-bold mb-1 uppercase tracking-wider">
          <span className={val < 0 ? "text-purple-600" : "text-gray-500"}>{leftLabel}</span>
          <span className={val > 0 ? "text-purple-600" : "text-gray-500"}>{rightLabel}</span>
        </div>
        <div className="relative h-3 w-full bg-gray-200 rounded-full border border-black overflow-hidden shadow-[inset_1px_1px_0px_0px_rgba(0,0,0,0.2)]">
          <div className="absolute top-0 bottom-0 left-0 bg-transparent z-[1]" style={{ width: "50%", borderRight: "2px solid black" }} />
          <div
  ...
  return (
    <div className="w-72 md:w-80 border-l-2 border-black bg-white flex flex-col h-full z-10 shadow-[-4px_0px_0px_0px_rgba(0,0,0,0.1)] overflow-y-auto shrink-0 transition-all duration-300">
      <div className="p-4 border-b-2 border-black bg-purple-50 shrink-0">
        ...
        <p className="text-xs text-gray-600 mt-1">Live cognitive model driving this session.</p>
      </div>
      ...
      <div className="p-5 flex-1 pb-20">
        {isLoading && !vector ? (
          <div className="animate-pulse space-y-4">
            {[1, 2, 3, 4].map(i => (
               <div key={i} className="h-12 bg-gray-200 rounded border-2 border-dashed border-gray-300" />
            ))}
          </div>
        ) : vector ? (
          ...
          <div className="text-sm text-gray-500 text-center font-bold">Failed to load profile.</div>
        )}
      </div>
    </div>
  );
